...
public class Lotto {
    ...
    /**
     * Checks if main method's String [] args array is empty or not.
     * If empty, jumps to a method for numbers input, else sends the array to be checked if it is valid.
     * 
    ...
    /**
     * Checks if user gave numbers in the command prompt while initializing the application and if those numbers are valid.
    ...
    public static int[] userInput(int [] array, int index, int pool) {
        ...
        for (int i = 0; i < array.length; i++) {
            ...
            if (same == true || array.length != index || highest > pool || lowest < 1) {
